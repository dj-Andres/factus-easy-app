...
export default function QuickCreditNoteListPage() {
  ...
  const sortValue = (note: QuickCreditNote, key: string): string | number | null | undefined => {
    switch (key) {
      case 'comprobante':
        return `${note.series}-${note.sequential}`
      case 'type':
        return creditNoteTypeLabel(note.credit_note_type)
      case 'customer':
        return note.customer_name ?? ''
      case 'original':
        return `${note.original_invoice_series}-${note.original_invoice_sequential}`
      case 'date':
        return note.emission_date ?? ''
      case 'status':
        return note.status
      case 'total':
        return note.total ?? 0
  ...
  const { sortedRows: sortedCreditNotes, toggle: toggleSort, reset: resetSort, indicator } = useColumnSort(
    creditNotes,
    sortValue,
  )
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 data-guide="page-title" className="text-xl font-semibold tracking-tight text-ink">Notas de Crédito</h1>
          <p className="mt-1 text-sm text-muted">{total} notas de crédito</p>
        </div>
        <div className="flex items-center gap-2">
          <Button data-guide="create-button" color="blue" onClick={() => navigate('/quick-credit-notes/new')}>
            Nueva Nota de Crédito
          </Button>
          <TourButton tourName="credit-notes" />
        </div>
      </div>
            ...
            <Table hoverable>
              <TableHead>
                <TableRow>
                  ...
                  <SortableTh label="Factura Original" onClick={() => toggleSort('original')} indicator={indicator('original')} />
